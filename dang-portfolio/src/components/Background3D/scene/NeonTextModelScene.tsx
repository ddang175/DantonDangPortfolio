...
const AnimatedLetter: React.FC<AnimatedLetterProps> = ({
  ...
  const phaseOffsetValue = useMemo(() => index * phaseOffset, [index, phaseOffset]);
  const secondaryPitchAmplitude = useMemo(() => pitchAmplitude * secondaryPitchMult, [pitchAmplitude, secondaryPitchMult]);
  const secondaryYawAmplitude = useMemo(() => yawAmplitude * secondaryYawMult, [yawAmplitude, secondaryYawMult]);
  const secondaryRollAmplitude = useMemo(() => rollAmplitude * secondaryRollMult, [rollAmplitude, secondaryRollMult]);
  ...
  useFrame((state) => {
    ...
    const secondaryPitch = Math.sin(time * secondaryPitchSpeed) * secondaryPitchAmplitude;
    const secondaryYaw = Math.cos(time * secondaryYawSpeed) * secondaryYawAmplitude;
    const secondaryRoll = Math.cos(time * secondaryRollSpeed) * secondaryRollAmplitude;
...
export const NeonTextModelScene: React.FC<NeonTextModelSceneProps> = ({
  ...
  const letterGroups = useMemo(() => 
    letters.map((letter, i) => (
      ...
      />
    )), [letters, fontSize, height, letterSpacing, material, enableMouseRotation, pitchSpeed, yawSpeed, rollSpeed, pitchAmplitude, yawAmplitude, rollAmplitude, phaseOffset, secondaryPitchMult, secondaryYawMult, secondaryRollMult, secondaryPitchSpeed, secondaryYawSpeed, secondaryRollSpeed]);
  ...
  useEffect(() => {
    ...
    // Set position and rotation for the group
    modelRef.current.position.set(...position);
    modelRef.current.rotation.set(...rotation);
    modelRef.current.scale.setScalar(scale);

    // Notify that the text scene is ready
